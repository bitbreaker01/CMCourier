...
### Probar TODOS los tipos (158)

> `services/practice_all.py` + la sección "probar todos los tipos" de `practice_pane.py`. El paso a paso operativo está en [`how-to/operator/probar-todos-los-tipos.md`](../how-to/operator/probar-todos-los-tipos.md).

El tiro único responde "¿este código sube?". Antes de una migración de miles con 40+ tipos, la pregunta real es otra: **¿TODOS los tipos que configuré suben bien con sus metadatos?** Probarlos de a uno es impracticable — y el error que buscás (un metadato que CM rechaza en UN tipo puntual) es justo el que se escapa si no los probás todos. La sección "probar todos los tipos" de `[0]` sube un documento por cada tipo y llena una tabla en vivo: `IDCM · tipo · OK/FALLA · razón`, con la razón CRUDA de CM en las fallas. Cierra con `N OK · M FALLA`; si hubo una sola falla, el aviso NO sale en verde.

**Por qué los metadatos se piden UNA vez por campo distinto.** El operador no tipea metadatos por documento: sería absurdo con 3000 documentos. `distinct_fields()` toma la **unión de los nombres canónicos** de las propiedades `usar` de todos los tipos a probar (`canonical_name` de `usable_properties`), y se pide **un valor por cada campo distinto**. Diez campos distintos ⇒ diez preguntas, aunque se suban miles. Para cada tipo, `build_properties` — la MISMA de 141 — arma sus propiedades con el subconjunto de esos valores que el tipo efectivamente usa, mapeando canónico → id de wire de ESE tipo (`cmis_property_ids`). La pantalla muestra, al lado de cada campo, en cuántos tipos se usa: el operador entiende qué está definiendo. Esta es una prueba de **esquema**, no de resolución: los valores los DICTA el operador, no salen de `field_sources` (para eso están `types check` y la migración real).

**Por qué un solo PDF, generado al vuelo.** `build_marked_pdf` genera UN PDF sintético (`build_text_pdf`, un content stream con texto Helvetica legible) y sus bytes se reusan como cuerpo de todos los uploads — lo único que cambia por tipo es el nombre (`PRUEBA-<IDCM>-<ts>.pdf`), la carpeta, el object type y los metadatos. El PDF lleva adentro, como texto legible, la marca de la prueba (fecha, operador, estación, entorno): cualquiera que lo abra en CM ve de qué se trata sin depender del nombre. La marca es genérica a propósito — la identidad del tipo vive en el nombre y en los metadatos, no adentro del PDF compartido.

**Alcance y limpieza.** Por defecto se prueban los `IDCM` que una migración real tocaría — los referenciados por `MapeoRVI_CM` que resuelven en el manifest (`build_practice_types` itera por `cm_codes`, no por `get_all`: un `IDCM` cuyo `IDRVI` colisionó con otra fila sigue siendo un tipo a probar). Un toggle "incluir tipos no mapeados" agrega el resto del manifest con al menos una propiedad `usar` — probar tipos que nadie migra es basura en CM, así que es opt-in. Como el tiro único, no toca `migration_log`; cada upload exitoso deja su `cm_object_id` y el botón "borrar los de prueba" (`delete_practice_objects`) los borra todos en lote, reusando el `delete_object` de 141. Un borrado individual que falla NO aborta el resto: los que quedaron se listan para limpiarlos a mano. El worker corre secuencial (concurrencia mínima: es una prueba, no una corrida de carga) con un `stop` cooperativo.
